...
@Ignore
public class ApplayoutDemoView extends VerticalLayout implements BeforeEnterObserver {
  ...
  private void openSettings() {
    ...
    cbSwipeOpen.getElement().setAttribute("title",
        "When enabled, you can open the menu by swiping the left border of the screen.");
    cbFixed.getElement().setAttribute("title",
        "When enabled, the header is fixed at the top so it never moves away.");
    cbReveals.getElement().setAttribute("title",
        "When enabled, the header slides back when scrolling back up.");
    cbCompact.getElement().setAttribute("title",
        "When enabled, the height of the header is set to 32px.");
    cbPersistent.getElement().setAttribute("title",
        "When enabled, the drawer will be opened in a non-modal way");
    cbBelowHeader.getElement().setAttribute("title",
        "When enabled, the drawer will be placed below the header");
    cbRightAlignment.getElement().setAttribute("title",
        "When enabled, the drawer will be right aligned");
    ...
    binder.forField(cbPersistent).bind(DemoSettings::isDrawerPersistent,
        DemoSettings::setDrawerPersistent);
    binder.forField(cbBelowHeader).bind(DemoSettings::isDrawerBelowHeader,
        DemoSettings::setDrawerBelowHeader);
    binder.forField(cbRightAlignment).bind(DemoSettings::isDrawerRightAlignment,
        DemoSettings::setDrawerRightAlignment);
    binder.setBean(this.settings);

    VerticalLayout content = new VerticalLayout(cbMenuVisible, cbSwipeOpen, cbFixed, cbReveals,
        cbCompact, cbPersistent, cbBelowHeader, cbRightAlignment);
  ...
  private Component[] createMenuItems() {
    ...
    return new Component[] {
        ...
        new MenuItem("No icon"), new MenuItem("No icon, spaced").configure(mi -> mi.setIconBlank()),
        ...
        new MenuItem("Content", VaadinIcon.BOOK, () -> showHamletContent())
            .setCommand(MouseButton.MIDDLE, () -> {
              getUI().ifPresent(
                  ui -> ui.getPage().executeJs("window.open(window.location.href, '_blank')"));
            }),
        miToggleSettings, miHello, new MenuItem("About", "cloud", () -> showContent("About")), // icon
                                                                                               // as
                                                                                               // string
        ...
        new MenuItem("SubMenu").setIcon("build").setOpened(true).add(
            new MenuItem("Hello Again", "inbox", () -> showContent("Hello Again!")),
            new MenuItem("And Again", () -> showContent("And Again!")),
            new MenuItem("SubMenu").setOpened(true)
                .add(new MenuItem("Hello Again", () -> showContent("Hello Again!")))
                .add(new MenuItem("And Again", () -> showContent("And Again!")))),
        new MenuSeparator("Separator"), new MenuItem("Item 1"), new MenuItem("Item 2"),
        new MenuItem("Item 3"), new MenuItem("Item 4"), new MenuItem("Item 5"),
        new MenuItem("Item 6"), new MenuSeparator(), new MenuItem("Item 7"), new MenuItem("Item 8"),
        new MenuItem("Item 9"), new MenuItem("Item 10"), new MenuItem("Item 11"),
  ...
  private void showContent(String content) {
    ...
    PaperCard pc = new PaperCard(label,
  ...
  public void beforeEnter(BeforeEnterEvent event) {
    // Check if theme parameter is set to "dark"
